...
const proxyCommand = document.querySelector("#proxy-command");
...
document.querySelector("#proxy-tab").addEventListener("click", () => {
    document.querySelector("#proxy-selection").innerHTML = "";
    rsg.setState({
        commandType: CommandType.Proxy,
        encoding: "None"
    });
});

document.querySelector("#test-tab").addEventListener("click", () => {
    document.querySelector("#test-selection").innerHTML = "";
    rsg.setState({
...
document.querySelector("#clear-tab").addEventListener("click", () => {
    document.querySelector("#clear-selection").innerHTML = "";
    rsg.setState({
...
const rsg = {
    ...
    selectedValues: {
        ...
        [CommandType.Proxy]: filterCommandData(rsgData.reverseShellCommands, { commandType: CommandType.Proxy })[0].name,
        [CommandType.Test]: filterCommandData(rsgData.reverseShellCommands, { commandType: CommandType.Test })[0].name,
        [CommandType.Clear]: filterCommandData(rsgData.reverseShellCommands, { commandType: CommandType.Clear })[0].name,
    },
    commandType: CommandType.ReverseShell,
    ...
    uiElements: {
        ...
        [CommandType.Proxy]: {
            listSelection: '#proxy-selection',
            command: '#proxy-command'
        },
        [CommandType.Test]: {
            listSelection: '#test-selection',
            command: '#test-command'
        },
        [CommandType.Clear]: {
            listSelection: '#clear-selection',
    ...
    getType: () => {
        if (rsg.commandType === 'Proxy') {
            let cmd_name = rsg.getSelectedCommandName();
            return proxy_listener_types[cmd_name];
        }
    ...
    getType: () => {
        if (rsg.commandType === 'Test') {
            let cmd_name = rsg.getSelectedCommandName();
            return test_listener_types[cmd_name];
        }
    ...
    getType: () => {
        if (rsg.commandType === 'Clear') {
            let cmd_name = rsg.getSelectedCommandName();
            return clear_listener_types[cmd_name];
        }
...
document.querySelector('#red-port').addEventListener('click', () => {
    rsg.setState({
        port: rsg.getPort() - 1
    })
})
...
document.querySelector('#copy-proxy-command').addEventListener('click', () => {
    rsg.copyToClipboard(proxyCommand.innerText)
})

document.querySelector('#copy-test-command').addEventListener('click', () => {
    rsg.copyToClipboard(testCommand.innerText)
})

document.querySelector('#copy-clear-command').addEventListener('click', () => {
    rsg.copyToClipboard(clearCommand.innerText)
})
